...
class PantryItem {
    ...
    static async findByUserId(userId, filters = {}) {
        let query = `SELECT *, expiry_date - CURRENT_DATE AS days_left FROM pantry_items WHERE user_id = $1`;
        const params = [userId];
        let paramCount = 1;
        if (filters.category) {
            paramCount++;
    ...
    static async getExpiringSoon(userId, days = 3) {
        const result = await db.query(
            `SELECT *, 
    ...
    static async getStats(userId){
        const result = await db.query(
            `SELECT 
                ...
                COUNT(*) FILTER (WHERE expiry_date IS NOT NULL AND expiry_date <= CURRENT_DATE + INTERVAL '7 days') AS expiring_soon_count
             FROM pantry_items
             WHERE user_id = $1`,
            [userId]
        );
