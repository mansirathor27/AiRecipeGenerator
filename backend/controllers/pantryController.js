...
export const getPantryItems = async(req, res, next) =>{
    try{
        ...
        const items = await PantryItem.findByUserId(req.user.id, {category, 
            is_running_low: is_running_low === 'true' ? true : undefined, search});
        res.json({
            success: true,
            data: { items }
        });
...
export const getExpiringItems = async(req, res, next) =>{
    try{
        const days = 3; // Default to 3 as per requirement
        const items = await PantryItem.getExpiringSoon(req.user.id, days);
        res.json({
            success: true,
            data: { items }
